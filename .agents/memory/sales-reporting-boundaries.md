...
Owner sales reporting should be built from verified order state and the authoritative credit ledger, while storefront funnel events should contain only an opaque event ID, event type, artwork identity, and timestamp. Never use browser success-page state as revenue evidence or include customer, prompt, order, or Stripe identifiers in the report.
...
**How to apply:** Preserve these source and privacy boundaries when adding report filters, exports, comparisons, or new funnel stages.