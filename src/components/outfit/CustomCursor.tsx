...
export function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => { mx = e.clientX; my = e.clientY; if (first) { first = false; cx = mx; cy = my; setVisible(true); } };
    const onLeave = () => setVisible(false);
    const onEnter = () => setVisible(true);
    const tick = () => { cx += (mx-cx)*0.22; cy += (my-cy)*0.22; cursor.style.transform = `translate3d(${cx}px,${cy}px,0)`; rafId = requestAnimationFrame(tick); };
