...
4. `scripts/catalog/Prepare-DisplayAssets.ps1` fully decodes only those 100 selections and creates lightweight review derivatives — scaled and re-encoded, capped at the public ceiling (`PUBLIC_DISPLAY_MAX_DIMENSION` in `scripts/catalog/display-contract.ts`) and never upscaled. They carry **no visible mark**: the only text this script draws is the site OG image, on its own fixed 1200x630 canvas. Per ADR-027, storefront display protection is the lossy re-encode, not a watermark band. Clean sources remain outside the public site and are never copied to `public`.
