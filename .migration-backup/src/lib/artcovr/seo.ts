...
type SearchIndexingEnvironment = {
  ...
  NEXT_PUBLIC_ARTCOVR_PRIVATE_STAGING?: string;
};

export function isSearchIndexingDisabled(
  env: SearchIndexingEnvironment = process.env as SearchIndexingEnvironment,
): boolean {
  return (
    env.ARTCOVR_ALLOW_INDEXING === "0" ||
    env.NEXT_PUBLIC_ARTCOVR_PRIVATE_STAGING === "1"
  );
}
