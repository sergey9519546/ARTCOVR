...
const generationBody = z.object({
  ...
  coverText: z
    .object({
      ...
      artistName: z.string().max(120).nullable().optional(),
    })
    .nullable()
    .optional(),
...
export function sendCustomerServiceError(res: Response, error: unknown) {
...
export async function rollbackReferenceUpload(
  id: string,
  objectKey: string,
  io = {
    removePrivate,
    removeRecord: async (referenceId: string) => {
      await db.delete(artcovrReferenceUploads).where(eq(artcovrReferenceUploads.id, referenceId));
    },
  },
) {
  try {
    await io.removePrivate([objectKey]);
    // Keep the tracking row until deletion is confirmed. Otherwise a storage
    // outage would leave a personal photo with no record available for cleanup.
    await io.removeRecord(id);
  } catch {
    // Never expose cleanup errors or replace the original upload failure.
    logger.warn({ referenceUploadId: id }, "Reference upload cleanup incomplete");
  }
}
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (
      !artworkId ||
      artworkId.length > 200 ||
      !getPublicArtworkById(artworkId)
    ) {
      res
        .status(400)
        .json({ code: "invalid_request", message: "artworkId is required." });
      return;
    }
    ...
    try {
      ...
      await db
        ...
        .where(eq(artcovrReferenceUploads.id, id));
      res
        .status(201)
        .set("Cache-Control", "private, no-store")
        .json({ referenceUploadId: id });
    } catch (error) {
      await rollbackReferenceUpload(id, objectKey);
      sendCustomerServiceError(res, error);
    }
  },
);
    ...
    if (!parsed.success) {
      res
        ...
        .json({
          ...
          message: "artworkId and prompt are required.",
        });
      return;
    }
    try {
      ...
      void runGeneration(job, getAuthenticatedUserId(req)).catch(() => {
        // A database outage can prevent even recording the failure. Leave no
        // unhandled rejection; the admission/status deadline recovers its slot.
        logger.error(
          { generationId: job.id },
          "Generation worker could not persist its status",
        );
      });
      res
        .status(202)
...
async function statusHandler(
  ...
  const id = String(
    req.query.generationId ?? req.body?.generationId ?? "",
  ).trim();
  if (!id) {
    res
      .status(400)
      .json({ code: "invalid_request", message: "generationId is required." });
    return;
  }
  try {
    res
      .set("Cache-Control", "private, no-store")
      .json(await generationStatus(id, getAuthenticatedUserId(req)));
  } catch (error) {
    sendCustomerServiceError(res, error);
  }
}
    ...
    try {
      ...
      if (!email) {
        res
          ...
          .json({
            code: "missing_email",
            message: "Your account needs a verified email address.",
          });
        return;
      }
      const since = new Date(Date.now() - 60 * 60_000);
      const recent = await db
        .select({ id: artcovrInquiries.id })
        .from(artcovrInquiries)
        .where(
          and(
            eq(artcovrInquiries.clerkUserId, userId),
            gte(artcovrInquiries.createdAt, since),
          ),
        );
      if (recent.length >= 5) {
        res
      ...
      res
        .status(201)
        .json({ inquiryId: id, createdAt: createdAt.toISOString() });
    } catch (error) {
      sendCustomerServiceError(res, error);
    }
  },
