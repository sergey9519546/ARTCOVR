...
test("private or blank prompts receive a descriptive label without reconstructing their text", () => {
  assert.equal(generationHistoryLabel({ phase: "purchased" }), "Generated edit");
  assert.equal(generationHistoryLabel({ phase: "preview", prompt: "  " }), "Generated preview");
  assert.equal(generationHistoryLabel({ phase: "purchased", prompt: "Keep the blue sky" }), "Keep the blue sky");
});
