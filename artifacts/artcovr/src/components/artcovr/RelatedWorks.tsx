  ...
  return (
    <section aria-labelledby="related-works" className="mt-24 border-t-2 border-current pt-5">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link href={`/archive?similar=${encodeURIComponent(seed.slug)}`} className="link-hover inline-flex min-h-11 shrink-0 items-center self-start text-[11px] font-bold uppercase tracking-[.1em]">
      ...
      <div role="group" aria-label="Explore similar artwork" className="mt-6 flex flex-wrap gap-x-6 gap-y-2 border-b border-current/20">
        {MODES.map(({ value, label }) => (
          <button
            ...
            onClick={() => { setMode(value); setVisibleCount(WORKS_PER_BATCH); }}
        ...
        {matches.length === 0 ? (
          ...
          <ul className="mt-6 grid grid-cols-2 gap-x-4 gap-y-10 md:grid-cols-3 lg:gap-x-6">
            {visibleWorks.map(({ artwork, reasons }) => (
              <li key={artwork.id}>
                <Link href={`/product/${artwork.slug}`} className="group block" aria-label={`Open ${artwork.title}`}>
