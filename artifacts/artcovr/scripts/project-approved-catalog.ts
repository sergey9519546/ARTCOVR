...
const approvedPath = path.resolve(projectRoot, "../../.migration-backup/catalog/approved-artworks.json");
...
if (checkOnly) {
  ...
  if (current.replace(/\r\n/g, "\n") !== serialized.replace(/\r\n/g, "\n")) {
    throw new Error("Public catalog projection is stale. Run pnpm --filter @workspace/artcovr run catalog:project.");
  }
} else {
