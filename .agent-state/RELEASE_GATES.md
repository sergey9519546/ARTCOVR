...
| **G7: Security Headers** | CSP, X-Frame-Options, noindex on staging | `bun run check:headers` | **NOT RUN** — no executable command in the Command / Check column — it names only `vercel.json` plus prose, so nothing could be executed and no status can be certified |
| **G8: Commerce Hardening** | Frozen prices, idempotent settle, dispute revoke/restore, convergent webhooks, dual-lane generation admission, dispute-pause entitlement credit, base-drift reconciliation | `bun run db:verify` | **NOT RUN** — requires credentials and a live/disposable PostgreSQL instance to apply and behaviourally verify migrations 0001–0011: the `supabase` CLI is not installed (spawn ENOENT); no SUPABASE_DB_URL / SUPABASE_SERVICE_ROLE_KEY / DATABASE_URL is present in the environment. A file's presence never proves a migration is applied. |
