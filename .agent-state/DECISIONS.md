...
## ADR-014 (2026-08-14): Dead Surface Removal Post-Commit (cleanup)
- **Context**: The `_to_delete/` directory (63 files: db/custom.db, prisma/schema.prisma, 47 unused shadcn/ui components, 2 hooks, duplicate catalog.ts, utils.ts, demo artwork jpgs) was staged aside in commit 27d5d51 but never referenced anywhere in the codebase. `outputs/metadata-atlas-src/` (54 files) was untracked scratch.
- **Decision**: `git rm -r _to_delete/` and deleted `outputs/`. Confirmed via full `npm run verify` that nothing depends on them.
- **Impact**: Working tree clean. All gates still green (_to_delete/ fully recoverable from 27d5d51 in git history).
